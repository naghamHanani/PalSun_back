...
// Function to generate data for all devices
async function generateDataForDevices() {
    return new Promise((resolve, reject) => {
        // Get the device IDs from the devices table
        const deviceQuery = 'SELECT deviceId FROM devices';
        mydb.query(deviceQuery, (error, results) => {
            if (error) {
                return reject(error);
            }
            ...
            // For each deviceId, generate 2 records: one at 00:00 and one at 12:00
            deviceIds.forEach(deviceId => {
                // Generate data for the first record (00:00 today)
                const data1 = generateSensorData();
                insertData.push([
                    null, deviceId, JSON.stringify(data1), JSON.stringify({}) // Empty object for total (can be replaced with actual total data)
                ]);

                // Generate data for the second record (12:00 today)
                const data2 = generateSensorData();
                insertData.push([
                    null, deviceId, JSON.stringify(data2), JSON.stringify({}) // Empty object for total (can be replaced with actual total data)
                ]);
            });

            const query = 'INSERT INTO devices_data (id, deviceId, setData, total) VALUES ?';
...
async function populateDeviceData() {
    try {
        const results = await generateDataForDevices();
...
module.exports = { populateDeviceData };
